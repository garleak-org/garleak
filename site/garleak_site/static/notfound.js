...
(function () {
  ...
  var m = location.pathname.match(/^(\/example)?\/(abs|sketch)\/(?:(paper|sketch):)?([^\/]+?)\/?$/);
  if (!m) return;
  var pre = m[1] || "", type = m[3] || (m[2] === "abs" ? "paper" : "sketch"), key = m[4];
  var arch = data[pre];
  if (!arch) return;
  ...
  var list = pre + (type === "paper" ? "/list/" : "/sketch/list/");
  ...
  var page = pre + (type === "paper" ? "/abs/" : "/sketch/") + n + "/";
