...
def test_full_build_of_both_archives(built):
    root, stats = built
    assert stats["pages"] > 250 and stats["pdfs"] == 1
    for path in ("/", "/about/", "/stages/", "/spec/", "/verify/", "/submit/", "/citecheck/", "/terms/",
                 "/moderation/", "/search/", "/list/", "/sketch/list/", "/graduated/",
                 "/list/astro.ga/recent/", "/list/astro.ga/2026-09/", "/sketch/list/astro.ga/recent/",
    ...
    assert (root / "list" / "astro.ga" / "feed.xml").is_file()
    assert not (root / "example" / "list" / "astro.ga" / "feed.xml").exists()
...
def test_noindex_rules_for_a_populated_archive(built_as_root):
    ...
    assert meta_robots(page(root, "/list/astro.ga/recent/")) == "noindex, follow"
...
def test_gated_paper_appears_at_t1(tmp_path, config):
    ...
    m.paper(9, [("1.0", "initial", "2026-09-10", BODY)], category="other.applied", families=())
    out = tmp_path / "_site"
...
def test_example_listing_matches_the_prototype(built):
    root, _ = built
    text = page(root, "/example/list/astro.ga/recent/")
...
def test_build_date_is_used_for_the_real_archive(tmp_path, config):
    ...
    assert "Recent submissions" in page(out, "/list/astro.ga/recent/")
    assert (out / "list" / "astro.ga" / "2027-01" / "index.html").is_file()
    assert "/list/astro.ga/recent/" in page(out, "/list/astro.ga/new/")
...
def test_recent_listing_is_not_a_date_window(tmp_path, config, maker):
    ...
    papers = page(out, "/list/astro.ga/recent/")
    assert "Test paper 1" in papers and "Test paper 2" in papers
    assert "An old idea worth testing." in page(out, "/sketch/list/astro.ga/recent/")
